...
public class EntityUtil {
	...
	public static void setBlockString(Block b, String blockData) {
		...
		EntityUtil.spawnFrame(b, face, Integer.parseInt(args[2]), (type == 389) ? true : false);
	}
	...
	public static HangingEntry getHangingEntry(DataType type, Entity e, UUID uuid, String remover) {
		if (e instanceof ItemFrame) {
			ItemFrame frame = (ItemFrame) e;
			
			return new HangingEntry(uuid, remover, type, e.getLocation().getBlock().getLocation(), Material.ITEM_FRAME, EntityUtil.getFace(frame.getAttachedFace()), frame.getItem().getType().toString());
		} else if (e instanceof Painting) {
			Painting paint = (Painting) e;
			
			return new HangingEntry(uuid, remover, type, e.getLocation().getBlock().getLocation(), Material.PAINTING, EntityUtil.getFace(paint.getAttachedFace()), paint.getArt().toString());
		}
