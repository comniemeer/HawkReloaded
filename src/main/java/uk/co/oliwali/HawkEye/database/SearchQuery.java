...
public class SearchQuery extends Thread {
	...
	public void run() {
		...
		sql.append((this.delete ? "DELETE FROM " : "SELECT * FROM "));
		...
		if (this.parser.players.size() >= 1) {
			...
			for (String player : this.parser.players) {
				boolean ignoredUser = player.contains("!");
				boolean specificUser = player.contains("*");
				
				player = player.replace("!", "").replace("*", "");
				
				for (Map.Entry<User, Integer> entry : DataManager.dbPlayers.entrySet()) {
					 if ((specificUser ? entry.getKey().getName().equalsIgnoreCase(player) : StringUtils.containsIgnoreCase(entry.getKey().getName(), player))) {
						 if (ignoredUser) {
		...
		if (this.parser.worlds != null) {
			...
			for (String world : this.parser.worlds) {
		...
		if (this.parser.actions != null && this.parser.actions.size() > 0) {
			...
			for (DataType act : this.parser.actions) {
				acs.add(act.getId());
		...
		if (this.parser.filters != null) {
			for (String filter : this.parser.filters) {
				args.add("data LIKE ?");
				binds.add("%" + filter + "%");
			}
		}
		...
		try {
			...
			ex.printStackTrace();
			this.callBack.error(SearchError.MYSQL_ERROR, "Error executing MySQL query: " + ex);
			return;
		} finally {
			try {
				if (res != null) {
				...
				this.callBack.error(SearchError.MYSQL_ERROR, "Unable to close SQL connection: " + ex);
			}
		}
		...
		// Run callback
		if (this.delete) {
			((DeleteCallback) this.callBack).deleted = deleted;
		} else {
