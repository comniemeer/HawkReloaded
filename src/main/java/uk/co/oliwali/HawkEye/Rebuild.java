...
public class Rebuild implements Runnable {
	...
	public void run() {
		...
		while (i < this.speed && this.rebuildQueue.hasNext()) {
			...
			DataEntry entry = this.rebuildQueue.next();
		...
		if (!this.rebuildQueue.hasNext()) {
			...
			this.session.setRollbackResults(undo);
			this.session.setEditSpeed(Config.DefaultEditSpeed);
			
			Util.sendMessage(this.session.getSender(), "&cRebuild complete, &7" + counter + "&c edits performed");
