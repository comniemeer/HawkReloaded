...
public class Config {
	...
	public static List<Material> BlockFilter = new ArrayList<Material>();
	...
	public static String DbServerTable;
	...
	public Config(HawkEye plugin) {
		...
		for (String type : config.getStringList("block-filter")) {
			BlockFilter.add(Material.getMaterial(type.toUpperCase()));
		}
		...
		ToolBlock = Material.getMaterial(config.getString("general.tool-block").toUpperCase());
		...
		DbServerTable = config.getString("mysql.server-table");
		...
		ServerName = config.getString("general.servername");
		BungeeCord = config.getBoolean("general.bungeecord");
