...
public class DisplayManager {
	...
	public static void displayPage(PlayerSession session, int page) {
		...
		for (int i = (page - 1) * maxLines; i < ((page - 1) * maxLines) + maxLines; i++) {
			...
			DisplayManager.sendLine(session.getSender(), "&cID:" + entry.getDataId() + " &7" + time + "&c" + entry.getPlayerName() + " &7" + entry.getType().getConfigName());
			DisplayManager.sendLine(session.getSender(), "&cLoc: &7" + entry.getWorld() + "-" + entry.getX() + "," + entry.getY() + "," + entry.getZ() + " &cData: &7" + entry.getStringData());
		}
