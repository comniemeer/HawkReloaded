...
public class BlockUtil {
	...
	public static String getBlockString(BlockState block) {
		if (block.getRawData() != 0) {
			return block.getType().toString() + ":" + block.getRawData();
		}
	...
	public static ItemStack itemStringToStack(String item, Integer amount) {
		...
		if (itemArr.length > 1) {
			return new ItemStack(Material.getMaterial(itemArr[0].toUpperCase()), amount, (itemArr[1].length() <= 3 ? Byte.parseByte(itemArr[1]) : Short.parseShort(itemArr[1])));
		}
		
		return new ItemStack(Material.getMaterial(itemArr[0].toUpperCase()), amount);
	}
	...
	public static String getBlockStringName(String blockData) {
		...
		if (blockArr.length > 1) {
			return Material.getMaterial(blockArr[0].toUpperCase()).name() + ":" + blockArr[1];
		} else {
			return Material.getMaterial(blockArr[0].toUpperCase()).name();
		}
	}
	...
	public static void setBlockString(Block block, String blockData) {
		String[] blockArr = blockData.split(":");
		Material type = Material.getMaterial(blockArr[0].toUpperCase());
		byte data = (blockArr.length > 1) ? Byte.parseByte(blockArr[1]) : 0;
	...
	public static Material getMaterialFromString(String string) {
		return Material.getMaterial(string.split(":")[0].toUpperCase());
	}
