...
public class InventoryUtil {
		...
		for (ItemStack item : inventory) {
			...
			if (!enchants.isEmpty()) {
				for (Entry<Enchantment, Integer> entry : enchants.entrySet()) {
					enchantments = enchantments + "-" + Enchantment.getByName(entry.getKey().getName()).toString() + "x" + entry.getValue();
				}
			}
	...
	public static String compressItem(ItemStack item, boolean isPositive) {
		...
		if (!enchants.isEmpty()) {
			for (Entry<Enchantment, Integer> entry : enchants.entrySet()) {
				enchantments += "-" + Enchantment.getByName(entry.getKey().getName()).toString() + "x" + entry.getValue();
			}
		}
	...
	public static ItemStack uncompressItem(String data) {
		...
		if (enchants.length > 0) {
			for (String s : enchants) {
				...
				if (types.length != 1) {
					Enchantment enchantment = Enchantment.getByName(types[0].toUpperCase());
					
					if (enchantment != null) {
						stack.addUnsafeEnchantment(enchantment, Integer.parseInt(types[1]));
					}
				}
			}
		}
	...
	public static void handleHolderRemoval(Player remover, BlockState state) {
