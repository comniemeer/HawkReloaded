...
public class MonitorPlayerListener extends HawkEyeListener {
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		DataManager.updatePlayerName(player);
			...
			switch (block.getType()) {
				case SOIL:
					if (event.getAction() == Action.PHYSICAL) {
						...
						if (HawkBlockType.getHawkBlock(rel.getType()).equals(HawkBlockType.plant)) {
				...
				case ENDER_CHEST:
					if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
						DataManager.addEntry(new DataEntry(player, DataType.OPEN_CONTAINER, loc, block.getType().toString()));
					}
	...
	public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event){
		...
		DataManager.addEntry(new BlockChangeEntry(event.getPlayer(), type, loc, loc.getBlock().getState(), event.getBucket()));
	}
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		if (InventoryUtil.isHolderValid(holder) && HawkEye.InvSession.containsKey(player)) {
			...
			DataManager.addEntry(new ContainerEntry((Player) event.getPlayer(), InventoryUtil.getHolderLoc(holder), data));
		}
	}
